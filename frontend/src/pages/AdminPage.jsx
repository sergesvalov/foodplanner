        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-pink-100 lg:col-span-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-2">Добавить пользователя</label>
                        <form onSubmit={addFamilyMember} className="bg-gray-50 p-4 rounded border border-gray-200">
                            <input type="text" required placeholder="Имя" className="w-full border rounded p-2 mb-4" value={newMemberName} onChange={e => setNewMemberName(e.target.value)} />
                            <div className="mb-4 flex gap-2 flex-wrap">
                                {COLORS.map(c => (
                                    <button type="button" key={c.name} onClick={() => setNewMemberColor(c.name)} className={`w-8 h-8 rounded-full ${c.bg} transition-all ${newMemberColor === c.name ? `ring-4 ${c.ring}` : 'opacity-60'}`} />
                                ))}
                            </div>
                            <button className="w-full bg-pink-500 text-white py-2 rounded font-bold">Добавить</button>
                        </form>
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-2">Список</label>
                        <ul className="space-y-2 max-h-60 overflow-y-auto bg-white rounded pr-1">
                            {family.map(member => (
                                <li key={member.id} className="flex justify-between items-center bg-white border border-gray-100 p-3 rounded shadow-sm">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold uppercase bg-${member.color}-500`}>{member.name[0]}</div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-indigo-100 lg:col-span-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <ul className="mb-3 max-h-40 overflow-y-auto border rounded p-1 text-sm">
                            {tgUsers.map(u => (
                                <li key={u.id} className="flex justify-between items-center p-2 border-b bg-white">
                                    <span>{u.name} <span className="text-gray-400 text-xs">({u.chat_id})</span></span>
