...
const AdminPage = () => {
  ...
  const saveToken = async () => {
    try {
        ...
        if (res.ok) alert("✅ Токен сохранен");
  ...
  const deleteTgUser = async (id) => {
    if(window.confirm("Удалить пользователя из рассылки?")) { 
  ...
  const triggerExport = async (endpoint) => {
    if(!window.confirm("Сохранить данные в файл?")) return;
    try {
  ...
  const triggerImport = async (endpoint) => {
    if(!window.confirm("Восстановить данные? Текущие записи будут заменены.")) return;
    try {
        const res = await fetch(endpoint, {method: 'POST'});
        const data = await res.json();
        if(res.ok) { alert("✅ Готово"); fetchAllData(); } 
      ...
      <div className="container mx-auto max-w-5xl p-8 pb-32">
        ...
        <div className="flex justify-between items-center mb-10 border-b pb-4">
            <div>
                ...
                <p className="text-gray-500">Настройки системы</p>
            </div>
            <button onClick={() => setIsAuthenticated(false)} className="text-red-600 font-medium px-4 py-2 hover:bg-red-50 rounded transition-colors">
                Выйти
            </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {/* === БЛОК 1: ТЕЛЕГРАММ БОТ (Вернул на место) === */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <div className="flex gap-2">
                            ...
                            <button 
                                onClick={saveToken} 
                                className="bg-indigo-600 text-white px-4 rounded text-sm font-bold hover:bg-indigo-700 transition-colors"
                            >
                                OK
                            </button>
                        </div>
                        <p className="text-xs text-gray-400 mt-2">
                            Введите токен, полученный от @BotFather
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <ul className="mb-3 max-h-40 overflow-y-auto border border-gray-200 rounded p-1 text-sm bg-gray-50 scrollbar-thin">
                            {tgUsers.length === 0 && <li className="text-gray-400 text-xs p-2 text-center">Нет пользователей</li>}
                            {tgUsers.map(u => (
                                <li key={u.id} className="flex justify-between items-center p-2 border-b border-gray-200 last:border-0 bg-white hover:bg-gray-50">
                        ...
                        <form onSubmit={addTgUser} className="flex gap-2">
                            ...
                            />
                            <button className="bg-green-600 text-white px-3 rounded hover:bg-green-700 font-bold shadow-sm">
                                +
                            </button>
                        </form>
                        <p className="text-[10px] text-gray-400 mt-1">Chat ID можно узнать у @userinfobot</p>
                    </div>
                </div>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <form onSubmit={addFamilyMember} className="bg-gray-50 p-4 rounded border border-gray-200">
                            <input 
                                type="text" required placeholder="Имя (напр. Папа)" 
                            ...
                            <button className="w-full bg-pink-500 text-white py-2 rounded hover:bg-pink-600 font-bold transition-colors shadow-sm">Добавить</button>
                        </form>
                    </div>
                        ...
                        <ul className="space-y-2 max-h-60 overflow-y-auto bg-white rounded pr-1 scrollbar-thin">
                            ...
                            {family.map(member => (
                                <li key={member.id} className="flex justify-between items-center bg-white border border-gray-100 p-3 rounded shadow-sm hover:border-pink-200 transition-colors">
                                    ...
                                    <button onClick={() => deleteFamilyMember(member.id)} className="text-gray-400 hover:text-red-500 font-bold px-2" title="Удалить">✕</button>
                                </li>
                            ))}
                        </ul>
                    </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 lg:col-span-2 flex flex-col gap-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="border p-4 rounded hover:border-blue-300 bg-blue-50/10">
                    ...
                    <div className="border p-4 rounded hover:border-orange-300 bg-orange-50/10">
                    ...
                    <div className="border border-indigo-100 bg-indigo-50/50 p-4 rounded">
                        <div className="font-bold mb-3 text-indigo-900">⚙️ Полный бэкап</div>
                        <div className="flex gap-3 text-sm">
                        ...
                        <p className="text-[10px] text-gray-400 mt-2">Токены, Пользователи, Семья</p>
                    </div>
                </div>
            </div>
